...
async function listPublicProducts(env) {
  const result = await env.DB.prepare(
    "SELECT id, name, price_cents, payment_url FROM products WHERE enabled = 1 ORDER BY id"
  ).all();
...
async function listAdminProducts(env) {
  const result = await env.DB.prepare(
    "SELECT id, name, price_cents, payment_url, enabled FROM products ORDER BY id"
  ).all();
  return json({ products: result.results });
}
...
async function login(request, env) {
  ...
  if (password.length < 8 || password.length > 128) return json({ error: "Senha invalida." }, 400);
...
async function updatePassword(request, env, session) {
  ...
  if (newPassword.length < 12 || newPassword.length > 128) {
    return json({ error: "A nova senha precisa ter pelo menos 12 caracteres." }, 400);
  }
...
<p class="secure">&#128274; Pagamento processado pelo Mercado Pago</p></main><script src="/app.js?v=1" defer></script></body></html>`;

const ADMIN_HTML = `<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Administra&ccedil;&atilde;o da m&aacute;quina</title><link rel="stylesheet" href="/styles.css"></head>
<body><main class="shell admin-shell"><header class="hero compact"><div class="brand">ADMINISTRA&Ccedil;&Atilde;O</div><h1>Produtos da m&aacute;quina</h1><p>Altere nomes, pre&ccedil;os e links do Mercado Pago.</p></header>
<section id="login-panel" class="panel"><h2>Entrar</h2><form id="login-form"><label>Senha<input id="login-password" type="password" autocomplete="current-password" required minlength="8"></label><button type="submit">Entrar</button></form></section>
<section id="admin-panel" class="panel hidden"><form id="products-form"><div id="admin-products"></div><button type="submit">Salvar produtos</button></form><hr><h2>Trocar senha</h2><form id="password-form"><label>Senha atual<input id="current-password" type="password" autocomplete="current-password" required></label><label>Nova senha<input id="new-password" type="password" autocomplete="new-password" minlength="12" required></label><button class="secondary" type="submit">Alterar senha</button></form><button id="logout" class="link-button" type="button">Sair</button></section>
<div id="message" class="message hidden" role="status"></div></main><script src="/admin.js?v=1" defer></script></body></html>`;
