...
const TEST_COMMAND_TTL_SECONDS = 120;
const DEVICE_ONLINE_SECONDS = 45;
...
const SECURITY_HEADERS = {
  "Content-Security-Policy": "default-src 'none'; script-src 'self' https://sdk.mercadopago.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://*.mercadopago.com https://*.mercadopago.com.br; connect-src 'self' https://*.mercadopago.com https://*.mercadopago.com.br; frame-src https://*.mercadopago.com https://*.mercadopago.com.br https://*.mercadolibre.com; base-uri 'none'; frame-ancestors 'none'; form-action 'self'",
...
async function route(request, env) {
  ...
  if (request.method === "POST" && path === "/api/brick/session") {
    requireSameOrigin(request);
    return createBrickSession(request, env);
  }
  if (request.method === "POST" && path === "/api/brick/payment") {
    requireSameOrigin(request);
    return createBrickPayment(request, env);
  }
...
async function machineIsOnline(env, deviceId = "machine-1") {
...
async function createBrickSession(request, env) {
  ...
  const now = Math.floor(Date.now() / 1000);
  const orderId = `brick-${now}-${crypto.randomUUID()}`;
  await env.DB.prepare(
    "INSERT INTO payment_orders (id, product_id, product_name, price_cents, status, created_at, updated_at) VALUES (?, ?, ?, ?, 'created', ?, ?)"
  ).bind(orderId, product.id, product.name, product.price_cents, now, now).run();
...
async function createBrickPayment(request, env) {
  ...
  if (!/^brick-\d+-[a-f0-9-]{20,}$/.test(orderId)) throw new HttpError(400, "Pedido invalido.");

  const order = await env.DB.prepare(
    "SELECT id, product_id, product_name, price_cents, status, device_command_id FROM payment_orders WHERE id = ?"
  ).bind(orderId).first();
  if (!order) throw new HttpError(404, "Pedido nao encontrado.");
  if (order.device_command_id) throw new HttpError(409, "Este pagamento ja liberou o produto.");

  const paymentMethodId = typeof formData.payment_method_id === "string" ? formData.payment_method_id : "";
  if (!paymentMethodId || paymentMethodId.length > 80) throw new HttpError(400, "Forma de pagamento invalida.");

  const payerInput = formData.payer && typeof formData.payer === "object" ? formData.payer : {};
  const payer = {};
  if (typeof payerInput.email === "string" && payerInput.email.length <= 160) payer.email = payerInput.email;
  if (typeof payerInput.first_name === "string" && payerInput.first_name.length <= 80) payer.first_name = payerInput.first_name;
  if (typeof payerInput.last_name === "string" && payerInput.last_name.length <= 80) payer.last_name = payerInput.last_name;
  if (payerInput.identification && typeof payerInput.identification === "object") {
    const type = typeof payerInput.identification.type === "string" ? payerInput.identification.type : "";
    const number = typeof payerInput.identification.number === "string" ? payerInput.identification.number : "";
    if (type && number && type.length <= 20 && number.length <= 40) payer.identification = { type, number };
  }
  if (!payer.email) throw new HttpError(400, "Informe o e-mail solicitado pelo Mercado Pago.");

  const origin = new URL(request.url).origin || PUBLIC_BASE_URL;
  const payload = {
    transaction_amount: Number(order.price_cents) / 100,
    description: String(order.product_name || "Produto vending").slice(0, 120),
  ...
  if (typeof formData.token === "string" && formData.token.length <= 200) payload.token = formData.token;
  if (formData.issuer_id !== undefined && formData.issuer_id !== null && String(formData.issuer_id).length <= 40) payload.issuer_id = String(formData.issuer_id);
  const installments = Number(formData.installments);
  if (Number.isInteger(installments) && installments >= 1 && installments <= 24) payload.installments = installments;
  ...
  if (!response.ok) {
    console.error(JSON.stringify({ event: "mercado_pago_brick_payment_error", status: response.status, error: data?.message || data?.error || "unknown" }));
    throw new HttpError(502, data?.message || "Mercado Pago recusou o pagamento.");
  }
...
async function createTestCommand(env, motor) {
  ...
  const now = Math.floor(Date.now() / 1000);
  await expireStaleTestCommands(env, deviceId, now);
  const device = await env.DB.prepare(
    "SELECT last_seen FROM devices WHERE device_id = ? AND enabled = 1"
  ).bind(deviceId).first();
  if (!device || Number(device.last_seen || 0) < now - DEVICE_ONLINE_SECONDS) {
    throw new HttpError(503, "A maquina esta offline. Nenhum teste foi colocado na fila.");
  }
  ...
  const mqttDelivered = await publishDeviceCommand(env, { id: result.meta.last_row_id, deviceId, motor });
  return json({ ok: true, delivery: mqttDelivered ? "mqtt" : "http_fallback" });
}

async function expireStaleTestCommands(env, deviceId, now) {
  await env.DB.prepare(
    "UPDATE device_commands SET status = 'expired' WHERE device_id = ? AND status IN ('pending', 'claimed') AND created_at < ? AND NOT EXISTS (SELECT 1 FROM payment_orders WHERE payment_orders.device_command_id = device_commands.id)"
  ).bind(deviceId, now - TEST_COMMAND_TTL_SECONDS).run();
}
...
async function getProductImage(path, env) {
  ...
  if (!/^product-\d+-[a-f0-9-]+\.(jpg|png|webp)$/.test(key)) {
    return new Response(null, { status: 404, headers: SECURITY_HEADERS });
  }
...
async function updateProducts(request, env) {
  ...
  for (const product of body.products) {
    ...
    if (!isValidProductId(id) || seen.has(id)) return json({ error: "Produto invalido." }, 400);
...
<title>${title}</title><link rel="stylesheet" href="/styles.css?v=7"></head>
...
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Escolha seu produto</title><link rel="stylesheet" href="/styles.css?v=7"></head>
...
<div class="shelf-guide" aria-hidden="true"><span>Deslize para o lado</span><span class="shelf-arrow">→</span></div><section id="products" class="products product-shelf" aria-live="polite"><div class="loading">Carregando produtos...</div></section>
<p class="secure">&#128274; Pagamento processado pelo Mercado Pago</p><p class="support-contact">Problemas com a compra? <a href="mailto:Eduardo.wakim@gmail.com">Entre em contato</a></p></main>
<div id="brick-modal" class="payment-modal hidden" role="dialog" aria-modal="true" aria-labelledby="brick-title"><div class="payment-sheet"><button id="brick-close" class="payment-close" type="button" aria-label="Fechar">&times;</button><div class="brand">NOVO PAGAMENTO</div><h2 id="brick-title">Pagamento no site</h2><p id="brick-subtitle" class="payment-subtitle"></p><div id="brick-message" class="brick-message"></div><div id="paymentBrick_container"></div><div id="statusScreenBrick_container"></div></div></div>
<script src="https://sdk.mercadopago.com/js/v2"></script><script src="/app.js?v=7" defer></script></body></html>`;

const ADMIN_HTML = `<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Administra&ccedil;&atilde;o da m&aacute;quina</title><link rel="stylesheet" href="/styles.css?v=7"></head>
...
<section id="admin-panel" class="panel hidden"><div class="shelf-guide admin-guide" aria-hidden="true"><span>Prateleira: deslize para ver os produtos</span><span class="shelf-arrow">→</span></div><form id="products-form"><div id="admin-products" class="admin-shelf"></div><button type="submit">Salvar produtos</button></form><hr><h2>Trocar senha</h2><form id="password-form"><label>Senha atual<input id="current-password" type="password" autocomplete="current-password" required></label><label>Nova senha (m&iacute;nimo 6 caracteres)<input id="new-password" type="password" autocomplete="new-password" minlength="6" required></label><button class="secondary" type="submit">Alterar senha</button></form><button id="logout" class="link-button" type="button">Sair</button></section>
<div id="message" class="message hidden" role="status"></div></main><script src="/admin.js?v=6" defer></script></body></html>`;

const STYLES = `:root{color-scheme:light;--ink:#182026;--muted:#657078;--paper:#fffdf8;--accent:#00a650;--accent-dark:#087a42;--line:#e7e2d8;--shadow:0 18px 50px rgba(30,35,32,.12)}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 15% 0,#dcffe9 0,transparent 34%),linear-gradient(145deg,#f7f4ec,#eef7f0);font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color:var(--ink)}.shell{width:min(1100px,calc(100% - 32px));margin:auto;padding:54px 0 32px}.hero{text-align:center;margin-bottom:34px}.hero.compact{margin-bottom:24px}.brand{display:inline-block;padding:7px 12px;border:1px solid #a7dcb9;border-radius:999px;color:var(--accent-dark);font-size:.76rem;font-weight:800;letter-spacing:.14em}.hero h1{font-size:clamp(2.1rem,7vw,4.2rem);line-height:.98;margin:18px 0 13px;letter-spacing:-.055em}.compact h1{font-size:clamp(2rem,6vw,3.3rem)}.hero p{color:var(--muted);font-size:1.06rem;margin:0}.products{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}.shelf-guide{display:none;align-items:center;justify-content:center;gap:10px;margin:-12px 0 14px;color:var(--muted);font-size:.88rem;font-weight:750}.shelf-arrow{display:inline-block;font-size:1.3rem;animation:shelf-nudge 1.4s ease-in-out infinite}@keyframes shelf-nudge{0%,100%{transform:translateX(0)}50%{transform:translateX(7px)}}.product,.panel{background:rgba(255,255,255,.88);border:1px solid rgba(255,255,255,.9);border-radius:26px;box-shadow:var(--shadow);backdrop-filter:blur(10px)}.product{padding:20px;display:flex;flex-direction:column;min-height:310px;min-width:0}.product-number{width:58px;height:58px;border-radius:18px;background:#e8fff0;display:grid;place-items:center;color:var(--accent-dark);font-weight:900;font-size:1.5rem}.product-image{width:100%;height:190px;object-fit:cover;border-radius:18px;background:#eef2ef}.product h2{font-size:1.3rem;margin:20px 0 6px}.price{font-size:1.85rem;font-weight:900;letter-spacing:-.04em;margin:auto 0 20px}.product button,form button{border:0;border-radius:15px;background:var(--accent);color:white;font:inherit;font-weight:800;padding:15px 18px;cursor:pointer;transition:.2s transform,.2s background}.product button:hover,form button:hover{background:var(--accent-dark);transform:translateY(-1px)}.product .new-payment{margin-top:10px;background:#24302a}.product .new-payment:hover{background:#111915}button:disabled{background:#b9c0bc;cursor:not-allowed;transform:none}.secure{text-align:center;color:var(--muted);font-size:.9rem;margin:24px 0 8px}.support-contact{text-align:center;color:var(--muted);font-size:.78rem;margin:0 0 6px}.support-contact a{color:var(--accent-dark);text-decoration:none;font-weight:700}.support-contact a:hover{text-decoration:underline}.loading,.empty{grid-column:1/-1;text-align:center;padding:50px;color:var(--muted)}.machine-maintenance{margin:0 0 24px;padding:22px 18px;background:#fff1f1;border:2px solid #c62828;border-radius:18px;color:#b42318;font-size:clamp(1.3rem,4.5vw,1.75rem);font-weight:900;line-height:1.2;box-shadow:0 10px 30px rgba(198,40,40,.16)}.admin-shell{max-width:1180px}.panel{padding:26px}.hidden{display:none!important}form{display:grid;gap:16px}label{display:grid;gap:7px;font-weight:700;font-size:.92rem}input{width:100%;border:1px solid var(--line);border-radius:12px;background:white;padding:13px 14px;font:inherit;color:var(--ink)}input:focus{outline:3px solid rgba(0,166,80,.16);border-color:var(--accent)}.admin-shelf{display:grid;grid-template-columns:repeat(4,minmax(230px,1fr));gap:14px;margin-bottom:18px}.admin-product{padding:16px;border:1px solid var(--line);border-radius:18px;display:grid;grid-template-columns:1fr;gap:12px;background:rgba(255,255,255,.7);min-width:0}.admin-product:first-child{padding-top:16px}.admin-product:last-child{border-bottom:1px solid var(--line)}.full{grid-column:1/-1}.check{display:flex;align-items:center;gap:9px}.check input{width:auto}.photo-row{display:flex;align-items:center;gap:12px}.photo-preview{width:76px;height:76px;object-fit:cover;border-radius:12px;background:#eef2ef}.remove-photo{border:0;background:transparent;color:#a12424;text-decoration:underline;cursor:pointer}.hint{color:var(--muted);font-size:.8rem;font-weight:500}.panel hr{border:0;border-top:1px solid var(--line);margin:28px 0}.secondary{background:#24302a}.link-button{margin-top:20px;background:transparent;border:0;color:var(--muted);text-decoration:underline;cursor:pointer}.message{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);background:#182026;color:white;border-radius:12px;padding:12px 18px;box-shadow:var(--shadow)}.payment-modal{position:fixed;inset:0;z-index:50;background:rgba(12,18,15,.58);display:flex;align-items:flex-start;justify-content:center;overflow:auto;padding:18px}.payment-sheet{position:relative;width:min(620px,100%);min-height:260px;margin:auto;background:white;border-radius:24px;padding:24px;box-shadow:0 24px 80px rgba(0,0,0,.28)}.payment-sheet h2{font-size:1.7rem;margin:16px 44px 4px 0}.payment-subtitle{margin:0 0 16px;color:var(--muted)}.payment-close{position:absolute;right:14px;top:12px;width:44px;height:44px;border:0;border-radius:50%;background:#eef2ef;color:#24302a;font-size:2rem;line-height:1;cursor:pointer}.brick-message{margin:10px 0;color:#9d2525;font-weight:700}.brick-message:empty{display:none}#paymentBrick_container,#statusScreenBrick_container{width:100%;min-height:1px}@media(max-width:650px){.shell{width:min(100% - 20px,1100px);padding-top:28px}.hero{margin-bottom:24px}.shelf-guide{display:flex}.product-shelf,.admin-shelf{display:flex;overflow-x:auto;gap:12px;scroll-snap-type:x mandatory;scroll-padding:10px;padding:4px 10px 16px;margin-left:-10px;margin-right:-10px;-webkit-overflow-scrolling:touch;overscroll-behavior-x:contain}.product-shelf::-webkit-scrollbar,.admin-shelf::-webkit-scrollbar{height:5px}.product-shelf>.product{flex:0 0 82%;scroll-snap-align:start}.product-shelf.shelf-hint>.product{animation:product-shelf-hint .8s ease-in-out 1}@keyframes product-shelf-hint{0%,100%{transform:translateX(0)}45%,70%{transform:translateX(-28px)}}.admin-shelf>.admin-product{flex:0 0 88%;scroll-snap-align:start}.product{min-height:255px}.admin-guide{margin-top:0}.payment-modal{padding:0}.payment-sheet{min-height:100vh;border-radius:0;padding:18px 12px}.payment-sheet h2{font-size:1.45rem}.payment-close{position:fixed;right:10px;top:8px;z-index:60}}@media(prefers-reduced-motion:reduce){.shelf-arrow{animation:none}.product-shelf.shelf-hint>.product{animation:none!important}}`;

const PUBLIC_JS = `const container=document.querySelector("#products");const money=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"});const modal=document.querySelector("#brick-modal");const brickClose=document.querySelector("#brick-close");const brickTitle=document.querySelector("#brick-title");const brickSubtitle=document.querySelector("#brick-subtitle");const brickMessage=document.querySelector("#brick-message");let paymentController=null;let statusController=null;let mpInstance=null;async function destroyBricks(){try{if(paymentController)await paymentController.unmount()}catch{}try{if(statusController)await statusController.unmount()}catch{}paymentController=null;statusController=null;document.querySelector("#paymentBrick_container").replaceChildren();document.querySelector("#statusScreenBrick_container").replaceChildren()}async function closeBrick(){await destroyBricks();modal.classList.add("hidden");document.body.style.overflow="";brickMessage.textContent=""}brickClose.addEventListener("click",closeBrick);modal.addEventListener("click",event=>{if(event.target===modal)closeBrick()});async function startCheckout(item,button){button.disabled=true;const original=button.textContent;button.textContent="Abrindo pagamento...";try{const response=await fetch("/api/checkout",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({product_id:item.id})});const data=await response.json();if(!response.ok)throw new Error(data.error||"Nao foi possivel abrir o pagamento.");location.assign(data.checkout_url)}catch(error){button.textContent=error.message;setTimeout(()=>{button.textContent=original;button.disabled=false},2500)}}async function showStatus(paymentId,builder){if(paymentController){try{await paymentController.unmount()}catch{}paymentController=null}document.querySelector("#paymentBrick_container").replaceChildren();brickTitle.textContent="Acompanhe seu pagamento";brickSubtitle.textContent="O Mercado Pago atualiza o status aqui, sem sair do site.";statusController=await builder.create("statusScreen","statusScreenBrick_container",{initialization:{paymentId},callbacks:{onReady:()=>{},onError:error=>{console.error(error);brickMessage.textContent="Nao foi possivel mostrar o status. O pagamento continua sendo processado."}}})}async function startBrick(item,button){button.disabled=true;const original=button.textContent;button.textContent="Carregando novo pagamento...";brickMessage.textContent="";try{await destroyBricks();const response=await fetch("/api/brick/session",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({product_id:item.id})});const session=await response.json();if(!response.ok)throw new Error(session.error||"Nao foi possivel iniciar o novo pagamento.");if(typeof MercadoPago!=="function")throw new Error("O modulo do Mercado Pago nao carregou. Tente novamente.");mpInstance=new MercadoPago(session.public_key,{locale:"pt-BR"});const builder=mpInstance.bricks();modal.classList.remove("hidden");document.body.style.overflow="hidden";brickTitle.textContent="Pague sem sair do site";brickSubtitle.textContent=session.product_name+" - "+money.format(session.amount);paymentController=await builder.create("payment","paymentBrick_container",{initialization:{amount:session.amount},customization:{paymentMethods:{bankTransfer:"all",creditCard:"all",debitCard:"all",prepaidCard:"all"}},callbacks:{onReady:()=>{},onError:error=>{console.error(error);brickMessage.textContent="O Mercado Pago encontrou um erro no formulario."},onSubmit:({formData})=>new Promise(async(resolve,reject)=>{brickMessage.textContent="";try{const payResponse=await fetch("/api/brick/payment",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({order_id:session.order_id,form_data:formData})});const payment=await payResponse.json();if(!payResponse.ok)throw new Error(payment.error||"Pagamento nao processado.");if(!payment.payment_id)throw new Error("Mercado Pago nao retornou o pagamento.");resolve();await showStatus(payment.payment_id,builder)}catch(error){brickMessage.textContent=error.message;reject(error)}})}})}catch(error){brickMessage.textContent=error.message;if(modal.classList.contains("hidden")){modal.classList.remove("hidden");document.body.style.overflow="hidden";brickTitle.textContent="Novo pagamento";brickSubtitle.textContent="Nao foi possivel iniciar o teste."}}finally{button.textContent=original;button.disabled=false}}async function load(){try{const response=await fetch("/api/products",{headers:{Accept:"application/json"}});if(!response.ok)throw new Error();const data=await response.json();container.replaceChildren();document.querySelector("#machine-maintenance")?.remove();if(data.machine_online===false){const maintenance=document.createElement("div");maintenance.id="machine-maintenance";maintenance.className="empty machine-maintenance";maintenance.textContent="⚠️ "+(data.machine_message||"Máquina temporariamente em manutenção.");container.before(maintenance)}if(!data.products.length){container.innerHTML='<div class="empty">Nenhum produto dispon\\u00edvel.</div>';return}for(const item of data.products){const card=document.createElement("article");card.className="product";let visual;if(item.image_url){visual=document.createElement("img");visual.className="product-image";visual.src=item.image_url;visual.alt=item.name;visual.loading="lazy"}else{visual=document.createElement("div");visual.className="product-number";visual.textContent=item.id}const title=document.createElement("h2");title.textContent=item.name;const price=document.createElement("div");price.className="price";price.textContent=money.format(item.price_cents/100);const button=document.createElement("button");button.type="button";button.textContent="Comprar agora";button.addEventListener("click",()=>startCheckout(item,button));const newButton=document.createElement("button");newButton.type="button";newButton.className="new-payment";newButton.textContent="Novo pagamento (teste)";newButton.addEventListener("click",()=>startBrick(item,newButton));card.append(visual,title,price,button,newButton);container.append(card)}if(data.products.length>1){container.classList.remove("shelf-hint");requestAnimationFrame(()=>container.classList.add("shelf-hint"));setTimeout(()=>container.classList.remove("shelf-hint"),950)}}catch{container.innerHTML='<div class="empty">N\\u00e3o foi poss\\u00edvel carregar os produtos. Tente novamente.</div>'}}load();`;

const ADMIN_JS = `const loginPanel=document.querySelector("#login-panel");const adminPanel=document.querySelector("#admin-panel");const message=document.querySelector("#message");const productsRoot=document.querySelector("#admin-products");function notify(text){message.textContent=text;message.classList.remove("hidden");setTimeout(()=>message.classList.add("hidden"),5000)}async function api(path,options={}){const response=await fetch(path,{...options,headers:{"Content-Type":"application/json",...(options.headers||{})}});const data=await response.json();if(!response.ok)throw new Error(data.error||"Erro inesperado.");return data}function field(label,type,value,cls=""){const wrap=document.createElement("label");if(cls)wrap.className=cls;wrap.append(document.createTextNode(label));const input=document.createElement("input");input.type=type;input.value=value;wrap.append(input);return{wrap,input}}async function uploadPhoto(box){const file=box.querySelector('[data-field="photo"]').files[0];if(!file)return;const response=await fetch('/api/admin/products/'+box.dataset.id+'/image',{method:"POST",headers:{"Content-Type":file.type},body:file});const data=await response.json();if(!response.ok)throw new Error(data.error||"Erro ao enviar foto.")}async function loadAdmin(){try{const data=await api("/api/admin/products");loginPanel.classList.add("hidden");adminPanel.classList.remove("hidden");productsRoot.replaceChildren();for(const product of data.products){const box=document.createElement("section");box.className="admin-product";box.dataset.id=product.id;const name=field("Nome","text",product.name);name.input.dataset.field="name";name.input.maxLength=60;const price=field("Preco (R$)","number",(product.price_cents/100).toFixed(2));price.input.dataset.field="price";price.input.min="0.01";price.input.step="0.01";const photo=field("Foto do produto (JPG, PNG ou WEBP, ate 2 MB)","file","","full");photo.input.dataset.field="photo";photo.input.accept="image/jpeg,image/png,image/webp";const enabled=document.createElement("label");enabled.className="check full";const check=document.createElement("input");check.type="checkbox";check.checked=Boolean(product.enabled);check.dataset.field="enabled";enabled.append(check,document.createTextNode("Produto disponivel"));box.append(name.wrap,price.wrap);if(product.image_url){const row=document.createElement("div");row.className="photo-row full";const preview=document.createElement("img");preview.className="photo-preview";preview.src=product.image_url;preview.alt="Foto atual";const remove=document.createElement("button");remove.type="button";remove.className="remove-photo";remove.textContent="Remover foto";remove.addEventListener("click",async()=>{try{await api('/api/admin/products/'+product.id+'/image',{method:"DELETE",body:"{}"});await loadAdmin();notify("Foto removida.")}catch(error){notify(error.message)}});row.append(preview,remove);box.append(row)}const test=document.createElement("button");test.type="button";test.className="secondary full";test.textContent="Testar motor "+product.id;test.addEventListener("click",async()=>{test.disabled=true;try{const result=await api('/api/admin/products/'+product.id+'/test',{method:"POST",body:"{}"});notify(result.delivery==="mqtt"?"Teste enviado por MQTT.":"MQTT indisponivel; teste aguardando o fallback HTTP.")}catch(error){notify(error.message)}finally{test.disabled=false}});box.append(photo.wrap,enabled,test);productsRoot.append(box)}}catch{loginPanel.classList.remove("hidden");adminPanel.classList.add("hidden")}}document.querySelector("#login-form").addEventListener("submit",async event=>{event.preventDefault();try{await api("/api/admin/login",{method:"POST",body:JSON.stringify({password:document.querySelector("#login-password").value})});document.querySelector("#login-password").value="";await loadAdmin();notify("Login realizado.")}catch(error){notify(error.message)}});document.querySelector("#products-form").addEventListener("submit",async event=>{event.preventDefault();const boxes=[...document.querySelectorAll(".admin-product")];const products=boxes.map(box=>({id:Number(box.dataset.id),name:box.querySelector('[data-field="name"]').value,price_cents:Math.round(Number(box.querySelector('[data-field="price"]').value)*100),enabled:box.querySelector('[data-field="enabled"]').checked}));try{await api("/api/admin/products",{method:"PUT",body:JSON.stringify({products})});for(const box of boxes)await uploadPhoto(box);await loadAdmin();notify("Produtos e fotos salvos.")}catch(error){notify(error.message)}});document.querySelector("#password-form").addEventListener("submit",async event=>{event.preventDefault();try{await api("/api/admin/password",{method:"PUT",body:JSON.stringify({current_password:document.querySelector("#current-password").value,new_password:document.querySelector("#new-password").value})});event.target.reset();notify("Senha alterada.")}catch(error){notify(error.message)}});document.querySelector("#logout").addEventListener("click",async()=>{await api("/api/admin/logout",{method:"POST",body:"{}"});location.reload()});loadAdmin();`;
